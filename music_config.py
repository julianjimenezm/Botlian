...
class MusicSet:
    ...
    async def resume(self, ctx):
        voice_client = get(self.bot.voice_clients, guild=ctx.guild)
        if voice_client == None:
            await ctx.channel.send("Bot is not connected to vocal channel")
            return
    ...
    async def pl(self, ctx):
        ...
        if voice_client == None or not voice_client.is_connected():
            await ctx.channel.send("Bot is not connected to vocal channel", delete_after=10)
            return
        
        player = self.get_player(ctx)
        if player.queue.empty():
            return await ctx.send('Not songs in the playlist')
